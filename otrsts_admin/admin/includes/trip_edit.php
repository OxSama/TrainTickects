<?php
	...
	if(isset($_POST['edit'])){
		...
		try{
			$stmt = $conn->prepare("UPDATE trips SET train_id=:train_id ,trip_date=:DepatureDate  , 
				 	trip_path=:trip_path, status=:status  WHERE id=:id");


			$stmt->execute(['train_id'=>$train_id ,'trip_date'=>$DepatureDate, 
				 	'trip_path'=>$path, 'status'=>$status ,'id'=>$id]);
