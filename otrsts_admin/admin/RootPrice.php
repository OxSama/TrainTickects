<?php
	...
	if(isset($_POST['add'])){
		...
		$conn = $pdo->open();
			...
			try{
				...
				$stmt = $conn->prepare("SELECT stations.train_id, stations.name, stations.id as station , trains.id from stations inner join trains on stations.train_id = trains.id and trains.id = ? and root = 1 order by station asc limit 1");
  ...
  //$train_id = $_GET['id'];
		...
		$_SESSION['error'] = 'Fill up Station price form first';
	}

	header("location: stations.php?id=$train_id");
