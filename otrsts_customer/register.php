<?php
	...
	if(isset($_POST)&&isset($_POST['signup'])){
		...
		$secret='6Ld4SaMeAAAAAJR_S4u6NsAJpYX6NER31wYxh-pm';
		$token = $_POST['g-token'];
		...
		if($response['success']=='1'&&$response['score']>=0.5){
			...
			$_SESSION['error'] = 'recaptcha error! Try again';
			...
			if($row['numrows'] > 0){
				...
				try{
					...
					$message = "
						...
						<a href='http://localhost/O/otrsts_customer/activate.php?code=".$code."&user=".$userid."'>Activate Account</a>
					";

					//Load phpmailer
		    		require '../otrsts_admin/vendor/autoload.php';
				    ...
				    try {
				        ...
				        $mail->setFrom('otrsts2021@gmail.com');
				        
				        //Recipients
				        $mail->addAddress($email);              
				        $mail->addReplyTo('otrsts2021@gmail.com');
				       
				        //Content
				        $mail->isHTML(true);                                  
				        $mail->Subject = 'Sudan railways Sign Up';
				        $mail->Body    = $message;
